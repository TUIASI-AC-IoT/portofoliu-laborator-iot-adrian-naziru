        ...
        while (1) {
            ...
            if (len < 0) {
                ...
                if (strcmp(rx_buffer, "GPIO4=0") == 0) {
                    ESP_LOGI(TAG, "Action: Turn LED ON");
                    gpio_set_level(GPIO_OUTPUT_IO, ON );
                } else if (strcmp(rx_buffer, "GPIO4=1") == 0) {
                    ESP_LOGI(TAG, "Action: Turn LED OFF");
                    gpio_set_level(GPIO_OUTPUT_IO, OFF );
                } else if (strncmp(rx_buffer, "SET_VALUE ", 10) == 0) {
                    /* maybe a PWM ? */
                    char *value_str = rx_buffer + 10;
                    int value = atoi(value_str);
                    ESP_LOGI(TAG, "Action: Set value to %d", value);
                  
                } else {
                    ESP_LOGI(TAG, "Unknown command: %s", rx_buffer);
                }
            }
